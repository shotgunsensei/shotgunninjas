...
const getSafeErrorResponse = (error: unknown): { message: string; status: number } => {
  ...
  if (errorMessage.includes('email not available')) {
    return { message: 'Unable to verify your account. Please try signing in again.', status: 400 };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: "price_1ShdqmLIaoqrzCsoYPKhXj7x",
          quantity: 1,
        },
      ],
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/workshop?success=true`,
      cancel_url: `${req.headers.get("origin")}/workshop?canceled=true`,
    });
