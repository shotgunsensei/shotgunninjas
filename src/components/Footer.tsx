...
const Footer = () => {
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success("Subscribed. You're in the loop.");
    setEmail("");
  };
    ...
    <footer className="border-t border-border bg-card/50">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand */}
          <div>
            <Link to="/" className="flex items-center gap-3 font-display text-lg font-bold text-gradient tracking-wider mb-4">
          ...
          {/* Products */}
          <div>
            <h4 className="font-display text-xs tracking-[0.2em] uppercase text-foreground mb-4">Platforms</h4>
            <div className="flex flex-col gap-2">
              {productLinks.map((link) => (
                <Link key={link.name} to={link.href} className="text-muted-foreground hover:text-primary transition-colors text-sm">
                  {link.name}
                </Link>
              ))}
            </div>
          ...
          {/* Company */}
          <div>
            <h4 className="font-display text-xs tracking-[0.2em] uppercase text-foreground mb-4">Company</h4>
            <div className="flex flex-col gap-2">
          ...
          {/* Email Signup */}
          <div>
            <h4 className="font-display text-xs tracking-[0.2em] uppercase text-foreground mb-4">Stay in the Loop</h4>
            <form onSubmit={handleSubscribe} className="flex gap-2">
              ...
              />
              <Button variant="hero" size="sm" type="submit">Go</Button>
            </form>
            {/* Social Icons */}
        ...
        <div className="border-t border-border mt-12 pt-8 text-center">
          <p className="text-muted-foreground text-sm">
            © {currentYear} Shotgun Ninjas Productions. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
